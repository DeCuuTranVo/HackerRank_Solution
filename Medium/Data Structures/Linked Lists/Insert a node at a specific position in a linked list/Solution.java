    ...
    // Write your code here
        if (llist == null) {
            ...
            if (position == 0) {
                newNode.next = head;
                head = newNode;  
            } else if (position == llistLength) { // if position == last + 1, insert to last.next
                tail.next = newNode;
            } else { // else, insert to that position
               // find previous node
               int prevNodePosition = position - 1;
               int nodeCounter = 0;
               SinglyLinkedListNode prevNode = head;
               while (nodeCounter < prevNodePosition) {
                   prevNode = prevNode.next;
                   nodeCounter++;
               }
               SinglyLinkedListNode posNode = prevNode.next;
